...
from zfs_tools.util import progressbar, SpecialPopen
...
class ZFSConnection:
        ...
        if self._dirty:
            ...
            stdout2 = subprocess.check_output(
                ...
                + [
                    ...
                    "-Hpr",
                    "-o",
                    ",".join(["name"] + properties),
                    "-t",
                    "all",
                ]
                + ([self._subset_name] if self._subset_name is not None else [])
            )
    ...
    def send(self, name, opts=None, bufsize=-1, compression=False, lockdataset=None):
        ...
        p = SpecialPopen(
            cmd, stdin=open(os.devnull), stdout=subprocess.PIPE, bufsize=bufsize
        )
        return p

    def receive(
        self, name, pipe, opts=None, bufsize=-1, compression=False, lockdataset=None
    ):
        if self._subset_name is not None and not name.startswith(self._subset_name):
            raise ValueError("Your connection is limited to " + self._subset_name)
        if not opts:
    ...
    def transfer(
        ...
        sndprg = self.send(
            s,
            opts=[] + fromsnapshot + send_opts,
            bufsize=bufsize,
            compression=compression,
            lockdataset=locksrcdataset,
        )
        sndprg_supervisor = Thread(
            target=lambda: queue_of_killables.put((sndprg, sndprg.wait()))
        )
        ...
        if showprogress:
            try:
                barprg = progressbar(
                    pipe=sndprg.stdout, bufsize=bufsize, ratelimit=ratelimit
                )
                barprg_supervisor = Thread(
                    target=lambda: queue_of_killables.put((barprg, barprg.wait()))
                )
        ...
        try:
            rcvprg = dst_conn.receive(
                d,
                pipe=barprg.stdout,
                opts=["-Fu"] + receive_opts,
                bufsize=bufsize,
                compression=compression,
                lockdataset=lockdstdataset,
            )
            rcvprg_supervisor = Thread(
                target=lambda: queue_of_killables.put((rcvprg, rcvprg.wait()))
            )
        ...
        allprocesses = set([rcvprg, sndprg]) | (
            set([barprg]) if showprogress else set()
        )
        while allprocesses:
